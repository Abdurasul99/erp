      ...
      <Card icon="📅" title={tt('Сезонный календарь')}>
        <div className="grid-4">
          {[
            ...
            [tt('Март'), tt('8 марта пик · Навруз'), '#22C55E'],
            [tt('Апрель'), tt('Свадьбы начинаются'), '#FF6B2B'],
            [tt('Май'), tt('Туристический сезон'), '#5B4FE8'],
            [tt('Июнь'), tt('Высокий B2C'), '#F59E0B'],
            [tt('Июль'), tt('Затишье B2C, активный B2B'), '#9094B0'],
            [tt('Август'), tt('Школа · корп. подарки'), '#0EA5E9'],
            [tt('Сентябрь'), tt('Новый сезон'), '#EC4899'],
            [tt('Октябрь'), tt('Подготовка к НГ'), '#22C55E'],
            [tt('Ноябрь'), tt('НГ-распродажи'), '#FF6B2B'],
            [tt('Декабрь'), tt('Пик НГ · 31 декабря'), '#5B4FE8'],
          ].map(([m, n, c]) => (
