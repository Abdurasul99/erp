  ...
  return (
    <>
      ...
      <Card icon="🆚" title={tt('Сравнение цен')}>
        <table>
          <thead><tr><th>{tt('Товар')}</th><th style={{ textAlign: 'right' }}>{tt('Мы')}</th><th style={{ textAlign: 'right' }}>{tt('Конк. A')}</th><th style={{ textAlign: 'right' }}>{tt('Конк. B')}</th><th>{tt('Позиция')}</th></tr></thead>
          <tbody>
            {[
              ...
              <tr key={r[0]}>
                <td style={{ fontWeight: 700 }}>{tt(r[0])}</td>
                ...
                <td>{r[4] === 'best' && <Badge tone="green">{tt('✓ Лучшая цена')}</Badge>}
                    {r[4] === 'mid' && <Badge tone="yellow">{tt('≈ В рынке')}</Badge>}
                    {r[4] === 'expensive' && <Badge tone="red">{tt('⚠️ Дороже')}</Badge>}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <Card icon="💪" title={tt('Наши сильные стороны')}>
          <div className="list">
            {['Доставка 24ч · быстрее на 2 дня', 'Возврат 100% без вопросов', 'Личный менеджер для B2B', 'Узбекская эстетика — туристы'].map(t => (
              <div key={t} className="list-item"><Badge tone="green">+</Badge><span style={{ fontSize: 13 }}>{tt(t)}</span></div>
            ))}
          </div>
        </Card>
        <Card icon="⚠️" title={tt('Где мы проигрываем')}>
          <div className="list">
            {['Нет точек в Самарканде/Бухаре', 'Меньший ассортимент дерева', 'Сайт менее удобный', 'Нет рассрочки 3/6/12'].map(t => (
              <div key={t} className="list-item"><Badge tone="red">−</Badge><span style={{ fontSize: 13 }}>{tt(t)}</span></div>
            ))}
          </div>
        </Card>
      </div>
