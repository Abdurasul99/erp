...
export default function AiAnalyze({ topic, branchId }) {
  ...
  const run = () => {
    ...
    api.post('/ai/analyze', { topic }, { params })
      .then(r => setAnalysis(r.data?.analysis || 'Пустой ответ от AI.'))
    ...
    <div style={{ marginTop: 16 }}>
      {!open ? (
        <button className="btn btn-primary" onClick={run}
          ...
          🤖 AI-анализ этих данных
        </button>
      ) : (
        <Card icon="🤖" title="AI-анализ"
          actions={<button className="btn btn-ghost btn-sm" onClick={() => setOpen(false)}>Скрыть</button>}>
          {loading ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--text2)', padding: '8px 0' }}>
              <span className="ai-spinner" /> AI анализирует ваши реальные цифры…
            </div>
          ) : error ? (
            <div style={{ color: 'var(--red)', fontWeight: 600 }}>
              ⚠️ {error}
              <button className="btn btn-ghost btn-sm" onClick={run} style={{ marginLeft: 10 }}>Повторить</button>
            </div>
          ) : (
