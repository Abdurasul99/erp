...
// Языковая директива для AI: отвечать на языке интерфейса (uz → узбекская латиница).
function aiLangRule(lang) {
  return lang === 'uz'
    ? "ЯЗЫК ОТВЕТА (ОБЯЗАТЕЛЬНО): отвечай ТОЛЬКО на узбекском языке ЛАТИНИЦЕЙ (O'zbek tili, lotin alifbosida — qat'iy lotin yozuvi, НЕ кириллица). Апостроф = U+0027 ('). Даже если вопрос задан на русском — пока интерфейс узбекский, отвечай по-узбекски латиницей."
    : 'ЯЗЫК ОТВЕТА: отвечай на русском языке.';
}

// Жёсткая «граница данных»: AI отвечает ТОЛЬКО по данным этой компании из промпта.
// Никакого интернета, внешних сайтов, рыночных бенчмарков и общих «лучших практик».
// У модели нет веб-доступа — это правило страхует от выдуманных внешних цифр/советов.
const AI_DATA_BOUNDARY = 'ГРАНИЦА ДАННЫХ (СТРОГО, ВАЖНЕЕ ОСТАЛЬНОГО): отвечай ИСКЛЮЧИТЕЛЬНО на основе данных этой компании, приведённых в этом промпте, и возможностей самой системы WareApp. ЗАПРЕЩЕНО использовать интернет, внешние сайты, новости, отраслевые и рыночные бенчмарки, «средние по рынку/индустрии», курсы валют, данные других компаний и любые общие «лучшие практики», которых нет в этих данных. Каждый вывод, прогноз и рекомендация должны вытекать ТОЛЬКО из приведённых цифр этой компании. Не ссылайся на внешние источники, сайты или сторонние сервисы и не называй их. Если данных для ответа не хватает — честно скажи об этом и подскажи, что заполнить или где посмотреть ВНУТРИ системы; НЕ подставляй и не выдумывай цифры/факты извне.';
...
app.post('/api/ai/analyze', auth(AI_ROLES), async (req, res) => {
  try {
    ...
    const sysChannels = aiLangRule(lang) + ' ' + AI_DATA_BOUNDARY + ` Ты — маркетинговый аналитик ERP для розницы в Узбекистане. По данным каналов привлечения дай: (1) какой канал приносит больше денег и почему (LTV, ROI, повторные), (2) куда перелить бюджет и от чего отказаться, (3) 1-2 действия. Кратко, только по цифрам. Если у канала нет расхода — отметь, что ROI не посчитать без ввода бюджета.`;
    const sys = topic === 'channels' ? sysChannels : aiLangRule(lang) + ' ' + AI_DATA_BOUNDARY + ` Ты — финансовый аналитик ERP-системы для розничного бизнеса в Узбекистане. Кратко и по делу. На основе реальных цифр компании дай: (1) короткий вывод о состоянии (2-3 предложения), (2) 2-3 конкретных действия. Без воды, без общих фраз. Суммы в сумах. Используй ТОЛЬКО приведённые цифры, не выдумывай.`;
...
app.post('/api/ai/suggest', auth(AI_ROLES), async (req, res) => {
  try {
    ...
    const prompt = (lang === 'uz'
      ? "Avvalgi javob asosida chakana biznes egasi nomidan 3 ta qisqa follow-up savol taklif qil. Savollar O'zbek tilida, lotin alifbosida bo'lsin. Savollar FAQAT shu kompaniyaning tizim ichidagi ma'lumotlari va bo'limlari haqida bo'lsin — tashqi mavzularsiz (bozor, raqobatchilar, internet). "
      : 'На основе предыдущего ответа предложи 3 коротких follow-up вопроса от лица владельца розничного бизнеса. Вопросы ТОЛЬКО про данные и разделы этой компании внутри системы — без внешних тем (рынок, конкуренты, интернет). ')
